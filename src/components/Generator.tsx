...
export default function Generator() {
  ...
  return (
    <div className="w-full max-w-xl mx-auto flex flex-col items-center gap-6">
      ...
      <div className="relative group w-full min-h-[80px] bg-gray-100 border rounded p-4 text-lg text-gray-800 transition shadow hover:shadow-lg cursor-pointer" onClick={handleCopy}>
        {text ? (
          <>
            <span>{text}</span>
            <span className="copy-icon" title="Copy">
              {copied ? (
                ...
                <svg className="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><rect x="9" y="9" width="13" height="13" rx="2" /><path d="M5 15V5a2 2 0 012-2h10" /></svg>
              )}
            </span>
          </>
        ) : loading ? (
          ...
          <span className="text-gray-400">Your funny placeholder text will appear here.</span>
        )}
      </div>
